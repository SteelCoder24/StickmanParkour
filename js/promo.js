  ...
  refetchAd(onAdEnd) {
	try {
		...
		this.skipInterval = setInterval(() => {
		  if (remainingTime <= 0) {
			this.skipButton.textContent = 'Skip Ad \u23E9'; // Update text to "Skip" ⏩
   ...
   updateAd() {
	...
	showRewardAd() {
		...
		try {
			...
			const skipTime = 8; //ad.type === "0" ? 5 : 10; // Skip button appears after this time
			...
			this.skipInterval = setInterval(() => {
			  if (remainingTime <= 0) {
				this.skipButton.textContent = 'Get Reward \u23E9'; // Update text to "Skip" ⏩
				this.skipButton.classList.add('enabled');
				this.skipButton.style.cursor = 'pointer'; // Enable cursor
				this.skipButton.onclick = () => {
				  this.closeAd();
